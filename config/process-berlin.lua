...
function node_function(node)
	...
	-- local network = node:Find("network");
	...
	local subway = node:Find("subway")
	local train = node:Find("train")
	local bus = node:Find("bus")
	local tram = node:Find("tram")
	local railway = node:Find("railway")
	...
	if amenity == "ferry_terminal" then
		...
		node:Attribute("class", "ferry")
		return
	end

	if highway == "bus_stop" and public_transport ~= "stop_position" and bus ~= "yes" then
		node:LayerAsCentroid("poi")
		SetNameAttributes(node)
		node:AttributeNumeric("rank", 12)
		node:Attribute("class", "bus")
		return
	end
	...
	if public_transport ~= "" then
		...
		elseif public_transport == "stop_position" then
			...
			if bus == "yes" then
				...
				node:Attribute("class", "bus")
				return
			elseif tram == "yes" and railway == "tram_stop" then
				node:LayerAsCentroid("poi")
				SetNameAttributes(node)
				node:AttributeNumeric("rank", 12)
				node:Attribute("class", "railway_light")
				return
			else
				-- TODO: ist there anything else to treat? 
				-- print("ooops stop_position:", public_transport, "HIGH", node:Find("highway"), "SUB", node:Find("subway"), "TRAIN", node:Find("train"), "BUS", node:Find("bus"), "TRAM", node:Find("tram"), 
				-- "RAILWAY", node:Find("railway"), node:Find("name"))
			end
			...
			-- 	print("opps catcha all:", public_transport, "HIGH", node:Find("highway"), "SUB", node:Find("subway"), "TRAIN", node:Find("train"), "BUS", node:Find("bus"), "TRAM", node:Find("tram"), 
			-- "RAILWAY", node:Find("railway"), node:Find("name"))
			-- end
		end
	...
	if (housenumber~="" and building ~="") or (housenumber~="" and amenity=="" and shop=="" and office=="" and craft=="") then
...
poiClassRanks   = { hospital=1, railway=2, bus=3, attraction=4, harbor=5, college=6,
					...
					park=13, library=14, police=15, post=16 }
poiKeys         = Set { "amenity", "tourism", "historic", "landuse", "information" }
...
function way_function(way)
	...
	if railway == "halt" or railway == "station" then
		print("SHOW RAILWAY STATION")
	end
